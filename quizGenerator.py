...
import xlrd, re, os, glob, docx, time, random
...
# from the xlsx get the words and then goto quizPrint() to print
# ask a lot of personalizing questions
def gen(fname):
...
def printQuiz(stu, wordList, opt, head, tail, answerOn = False): # print the quiz beautifully into docx files
    ...
    #h2.add_run('版本：' + str(stu + 1)).font.name = 'MicrosoftJhengHei'
    
    h3 = doc.add_heading('', level=4)
    h3.add_run('日期：' + currentDate, style='Song')
    ...
    if answerOn == False:
        #   默写纸_高考考纲词汇_1_
        path = '_'.join(['默写纸', sourceName, rangeName, versionName, currentTime]) + '.docx'
    else:
        path = '_'.join(['答案', sourceName, rangeName, versionName, currentTime]) + '.docx'
    
    if not os.path.exists(binPath[1]):
        os.makedirs(binPath[1])

    doc.save(binPath[1] + path)
    print('生成成功：' + binPath[1] + path)
